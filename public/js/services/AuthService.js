...
app.service('authService', function($http, $q, $rootScope) {  
                             ...
                             this.exists = function(email) {
                                             var request = $http({ method: "get",
                                                                   url: String.format("/api/auth/exists/{0}", email) });
                                             return (request.then(handleSuccess, handleError));
                                           };

                             this.resetPassword = function(email) {
                               console.log(String.format("/api/auth/reset/{0}", email));
                               var request = $http({ method: "put",
                                                     url: String.format("/api/auth/reset/{0}", email) });
                               return (request.then(handleSuccess, handleError));
                             };

                             // I get all of the friends in the remote collection.
                             this.signIn = function(email, password) {
                               var passwordHash = String.md5(password);
                               console.log(String.format("/api/auth/with/{0}/and/{1}", email, passwordHash));
                               var request = $http({ method: "get",
                                                     url: String.format("/api/auth/with/{0}/and/{1}", email, passwordHash) });
